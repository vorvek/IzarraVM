...
fn direct_memory_alu_paging_and_cross_page_exits_precede_flags_and_memory_mutation() {
    for (target, user_page, expected_cross_page, expected_unavailable) in [
        ...
        let interp_fault =
            interp.execute_decoded(&decoded, &mut interp_bus, &mut InstructionWork::default());
...
/// Window-shrink M8: a leftover Seen after an overlapping span write must compile the NEW
/// opcode. `compile` reads `decode_cache.get`, so a live line of the old bytes is the stale
/// path. The decode door has to kill that line; the refill is from current RAM.
#[cfg(any(
    all(target_os = "windows", target_arch = "x86_64"),
    all(target_os = "linux", target_arch = "x86_64")
))]
#[test]
fn leftover_seen_after_span_write_compiles_the_new_opcode() {
    let mut cpu = fresh();
    let mut memory = vec![0u8; 0x1000];
    memory[0x200..0x20f].fill(0x90);
    memory[0x20f] = 0xf4;
    let mut bus = TestBus::with_memory(memory);

    let mut linear = 0x200u32;
    while linear < 0x210 {
        cpu.registers.eip = linear;
        cpu.begin_instruction();
        let insn = cpu.fetch_decoded(&mut bus, linear).expect("warm neighbor");
        linear = linear.wrapping_add(u32::from(insn.len));
    }

    let neighbor = jit::direct::key_for(&cpu, 0x200, true).expect("neighbor key");
    assert!(matches!(
        cpu.jit_direct.probe(neighbor),
        jit::direct::BlockProbe::Interpret
    ));
    assert!(matches!(
        cpu.jit_direct.probe(neighbor),
    ...
    let compilation = jit::direct::compile(&mut cpu, 0x200, true).expect("neighbor block");
    assert!(
        compilation.span.key.physical <= 0x204
            && compilation.span.key.physical + u32::from(compilation.span.guest_len) > 0x204,
        "neighbor span must cover the leftover Seen physical"
    );
    cpu.jit_direct
        .install(&compilation)
        .expect("neighbor installs");

    let seen = jit::direct::key_for(&cpu, 0x204, true).expect("seen key");
    assert!(matches!(
        cpu.jit_direct.probe(seen),
        jit::direct::BlockProbe::Interpret
    ));

    cpu.registers.eip = 0x204;
    cpu.begin_instruction();
    cpu.fetch_decoded(&mut bus, 0x204).expect("warm NOP");
    assert_eq!(
        cpu.decode_cache.get(0x204, true).expect("NOP line").opcode,
        0x90
    );

    bus.memory[0x204] = 0x48;
    cpu.note_code_write(0x204, 1);
    cpu.prefetch.invalidate();
    ...
    assert!(
        cpu.decode_cache.get(0x204, true).is_none(),
        "the overlapping write must kill the old decode line"
    );
    assert_eq!(bus.memory[0x204], 0x48);

    cpu.registers.eip = 0x204;
    cpu.begin_instruction();
    cpu.fetch_decoded(&mut bus, 0x204).expect("refill DEC");
    assert_eq!(
        cpu.decode_cache.get(0x204, true).expect("DEC line").opcode,
        0x48
    );
    let compilation = jit::direct::compile(&mut cpu, 0x204, true).expect("new opcode block");
    cpu.jit_direct
        .install(&compilation)
        .expect("new opcode installs");
}
