...
pub(crate) fn render(
    ...
    let _ = writeln!(
        out,
        "# izarravm-ipe-window-trace-v1 window_entries={window_entries}"
    );
    out.push_str("window_index,entries,direct_insns,ipe,kind\n");
    ...
    let _ = writeln!(
        out,
        "# totals windows={} entries={total_entries} direct_insns={total_insns} ipe={total_ipe:.6}",
