...
impl GuiApp {
    ...
    fn controls_ui(&mut self, ui: &mut egui::Ui) {
        ...
        let (mode, speed, floppy_accesses, c_accesses) = match &self.emu {
            Some(emu) => {
                let f = emu.frame.lock().expect("frame snapshot poisoned");
                (f.mode, f.speed_ratio, f.floppy_accesses, f.c_accesses)
            }
            None => (None, 0.0, self.floppy_access_seen, self.c_access_seen),
        };
