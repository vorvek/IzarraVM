...
fn png_is_created_with_the_guest_pixels() {
    ...
    let path = save_png_with_stem(&sample_frame(), &directory, "IzarraVM_test").unwrap();
...
fn a_same_timestamp_uses_a_collision_suffix() {
    ...
    let first = save_png_with_stem(&sample_frame(), &directory, "IzarraVM_same").unwrap();
    let second = save_png_with_stem(&sample_frame(), &directory, "IzarraVM_same").unwrap();
...
fn an_invalid_frame_does_not_create_the_directory() {
    ...
    assert!(matches!(
        save_png_with_stem(&frame, &directory, "IzarraVM_bad"),
