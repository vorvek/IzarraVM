...
fn an_empty_census_renders_two_empty_lists_rather_than_nothing() {
    ...
    let json = mode_census_json(&ModeCensus::default(), &DistiraCensus::default());
...
fn a_vga_row_carries_every_key_field_and_its_count() {
    ...
    let json = mode_census_json(&vga, &DistiraCensus::default());
    let row = &json["vga"][0];
...
fn a_distira_row_carries_its_size_and_count() {
    ...
    let json = mode_census_json(&ModeCensus::default(), &distira);
