...
#[test]
fn inline_resolution_reveals_run_budgeted_where_the_old_resolver_collapsed_to_line_1693() {
    if std::env::var_os("IZARRAVM_RIPROFILE_INLINE_CHILD").is_some() {
        inline_chain_check_in_this_process();
        return;
    }
    let output = spawn_riprofile_child(
        "riprofile::tests::inline_resolution_reveals_run_budgeted_where_the_old_resolver_collapsed_to_line_1693",
...
fn inline_chain_check_in_this_process() {
    ...
    // `machine/src/run.rs:1693` (the call `cpu.run_budgeted(&mut bus,
    ...
    // joined `try_poll_skip`. THIS PIN DRIFTS WITH `machine/src/run.rs`'s line
    ...
    while offset < u64::from(size) {
        ...
        if !old_line.contains("run.rs:1693") {
            continue;
        }
    ...
    let (addr, innermost, old_line) = best.unwrap_or_else(|| {
        panic!(
            "no address in run_until_tick's {size:#x}-byte extent reproduced the \
             defect example: OLD resolver collapsing to machine/src/run.rs:1693 while \
    ...
    assert_eq!(
        old_line, "crates\\izarravm-machine\\src\\run.rs:1693",
