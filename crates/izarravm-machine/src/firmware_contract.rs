...
pub(super) fn install_boot_memory(memory: &mut Memory, mode: GswMode) -> Result<(), BusError> {
    ...
    // Vectors 60h-6Fh are deliberately absent from this list. IBM PC AT
    // Technical Reference, System BIOS 5-5, assigns 60h-67h to "user program
    // interrupts" and 68h-6Fh to "Not used"; the system BIOS initializes
    // neither group, leaving all sixteen at 0000:0000. Resident software takes
    // a slot by scanning that range for the first null entry, so seeding them
    // with ROM IRET stubs makes the scan report that no free vector exists.
    // INT 67h belongs to the same rule: the EMS manager overwrites the slot at
    // install time without reading it, and an application distinguishes "no
    // manager" from "manager present" by the null vector.
    for vector in [
        ...
        0x45, 0x48, 0x49, 0x4a, 0x58, 0x59, 0x5a, 0x5b, 0x5c, 0x5d, 0x5e, 0x5f, 0x78, 0x79, 0x7a,
        0x7b, 0x7c, 0x7d, 0x7e, 0x7f, 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0xe0, 0xe4, 0xef,
