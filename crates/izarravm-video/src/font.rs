...
//! Seeded into the writable font store on `Vga`; the text renderer reads the
//! active table. 256 glyphs per table, one byte per font scanline (bit 7 = left
//! pixel).
/// The 8x8 ROM font (256 glyphs, 8 bytes each).
pub const VGAFONT_8X8: [u8; 2048] = [
...
/// The 8x14 ROM font (256 glyphs, 14 bytes each).
pub const VGAFONT_8X14: [u8; 3584] = [
...
/// The 8x16 ROM font (256 glyphs, 16 bytes each). The mode-03h default.
pub const VGAFONT_8X16: [u8; 4096] = [
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
