...
#
# 2026-08-18, Doom realtics bands split by era -- NOT a re-pin. The
# 2026-08-10 storage-charge changes cut Doom's charged I/O stall over these
# budgets (0.996 -> 0.171 guest seconds on the 486 row), so the timedemo now
# completes in fewer realtics while gametics stays 2134. The fixture
# scoreboard shifted its bands the same day (doom-486 2900-3050 -> 2814-2964,
# doom-586 970-1040 -> 951-1021: down 86 and 19 tics, width and margins
# preserved). This script's bands were the pin-era ones and were never moved,
# so every candidate since 2026-08-10 died on the doom-486 warmup: measured
# here today, the pinned tree's build reports 2969 / 998 and a current main
# build reports 2883 / 979 -- the shift exactly, on the same fixture.
#
# The pin does NOT move for this. The baseline exists to reconstruct a
# known-good comparator from tree 5aabe720 via git archive; that tree is still
# buildable and still a valid comparator, and its rt/coverage floors are
# MASTER_CLOCK-sensitive ratchets that a re-pin would oblige a full
# re-measurement of (see the standing rule above). What broke is only the
# realtics identity check, which asserts that a build ran the SAME demo in the
# environment it was built for -- and the two roles now legitimately belong to
# two different eras. So the band is per-era: minimum/maximum_realtics carry
# the current (post-storage-charge) band, baseline_minimum/maximum_realtics
# carry the pin-era band, and Invoke-Observation picks by role. Both stay as
    ...
    switch ($Name) {
        "doom-486" {
            return [pscustomobject][ordered]@{
                ...
                # Realtics is an identity check, not a ratchet, and the two
                # roles sit either side of the 2026-08-10 storage-charge shift
                # (-86 tics here). Candidate band = the fixture scoreboard's
                # current band; baseline band = the pin-era band it replaced,
                # same width and same margins around 2969.
                minimum_realtics = 2814
                maximum_realtics = 2964
                baseline_minimum_realtics = 2900
                baseline_maximum_realtics = 3050
            }
        }
        "doom-586" {
            return [pscustomobject][ordered]@{
                ...
                # Same 2026-08-10 shift, -19 tics on this row. The old
                # 970-1040 band did not actually reject a current build (979
                # scrapes in 9 tics above the floor), but it was stale for the
                # same documented cause and had lost its lower margin, so it
                # moves with the scoreboard rather than waiting to fail.
                minimum_realtics = 951
                maximum_realtics = 1021
                baseline_minimum_realtics = 970
                baseline_maximum_realtics = 1040
            }
        }
        "quake-586" {
            return [pscustomobject][ordered]@{
                ...
                baseline_minimum_realtics = $null
                baseline_maximum_realtics = $null
            }
        }
...
function Invoke-Observation(
    ...
    # v1 is the pinned baseline's schema (fdf32ef2); v2 added fields additively
    # (92a6c493 Katea streaming, then the 2026-08-21 CD columns). The gate reads
    # only fields present in both, so either spelling is acceptable per role.
    if (($sample.schema -ne "izarravm-hdd-profile-v1" -and
    ...
    if ($Policy.name.StartsWith("doom-", [StringComparison]::Ordinal)) {
        # The baseline is reconstructed from the pinned tree and predates the
        # 2026-08-10 storage-charge shift, so it is held to the pin-era band;
        # every other role is a current build and is held to the current one.
        $realticsMinimum = $Policy.minimum_realtics
        $realticsMaximum = $Policy.maximum_realtics
        if ($Role -ceq "baseline") {
            $realticsMinimum = $Policy.baseline_minimum_realtics
            $realticsMaximum = $Policy.baseline_maximum_realtics
        }
        if (-not $captureProofArtifacts -and
            ($null -eq $sample.timedemo -or $sample.timedemo.gametics -ne 2134 -or
             $sample.timedemo.realtics -lt $realticsMinimum -or
             $sample.timedemo.realtics -gt $realticsMaximum)) {
