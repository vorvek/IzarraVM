...
<#
.SYNOPSIS
GATE 1 for D-elision B: wolf3d-586, two binaries, n=8 ABBA pairs.

.DESCRIPTION
No new knob. OFF is main at 56f90626 (A only). ON is this PR (A+B). Identity
first (one OFF, one ON), then 8 interleaved A/B/B/A pairs. Pairing is
...
Identity (hash, guest clocks, bus clocks) is a STOP. Wall: NEUTRAL does not
decline; sign worse than 4/8 or mean-wall ratio < 1.0 is a STOP.

See dev_docs/specs/2026-08-29-d-elision-b-design.md section 7.4-7.5.
#>

param(
    [Parameter(Mandatory)][string]$OffExecutable,
    [Parameter(Mandatory)][string]$OnExecutable,
...
$lockPath = Join-Path $bench "locks\d-elision-b-ladder.lock"
$expectedHash = "e33418bbd34c13ad9c99e23c3d1cddb68df08beecec8676378557a4cc102f963"

if (-not (Test-Path -LiteralPath $OffExecutable)) { throw "Missing OFF executable: $OffExecutable" }
if (-not (Test-Path -LiteralPath $OnExecutable)) { throw "Missing ON executable: $OnExecutable" }
...
"$PID d-elision-b-ladder" | Set-Content -LiteralPath $lockPath -NoNewline
...
function Invoke-Leg([string]$Arm, [string]$Label) {
    $exe = if ($Arm -eq "1") { $OnExecutable } else { $OffExecutable }
...
try {
    ...
    Write-Host "IDENTITY pair (OFF=$OffExecutable then ON=$OnExecutable)..."
    ...
    Write-Host ("IDENTITY OFF wall={0:N3}s ON wall={1:N3}s hash={2}" -f `
            $off.wall_s, $on.wall_s, $off.frame_sha256)
    ...
    Write-Host "WALL ladder: $Pairs ABBA pairs on wolf3d-586 (NEUTRAL does not decline; regression is a STOP)"
    ...
    for ($p = 1; $p -le $Pairs; $p++) {
        ...
        foreach ($step in @(@("0", "off"), @("1", "on"), @("1", "on"), @("0", "off"))) {
            ...
            $legs.Add($leg)
            $pairLegs.Add($leg)
            Write-Host ("  wall {0,9:N3}s  cpu {1}/{2}" -f `
                    $leg.wall_s, $leg.cpu_before, $leg.cpu_after)
        }
    ...
    $regression = ($signWins -lt 4) -or ($meanRatio -lt 1.0)
    $label = if ($regression) {
        "WALL_REGRESSION (STOP)"
    }
    elseif ($minRatio -ge 1.060 -and $signWins -eq $signN) {
        "WALL_LARGE (merge; leftover already default ON)"
    }
    elseif ($minRatio -ge 1.020 -and $signWins -ge 7) {
        "WALL_REAL_AND_SMALL (merge; leftover already default ON)"
    }
    else {
        "WALL_NEUTRAL (merge; leftover already default ON)"
    }
    ...
    $summary = [pscustomobject]@{
        ...
        merge_bar          = "identity STOP; wall regression STOP (sign < 4/8 or mean-wall ratio < 1.0); NEUTRAL does not decline"
    ...
    if ($regression) { throw "Wall regression STOP. See $OutDir\GATE.json" }
}
finally {
    if (Test-Path -LiteralPath $lockPath) {
