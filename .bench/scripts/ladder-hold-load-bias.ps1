...
<#
.SYNOPSIS
GATE 1 for IZARRAVM_DIRECT_HOLD_LOAD_BIAS: wolf3d-586, one binary, two arms, n=8 ABBA pairs.
...
cpu_before > 8 or cpu_after > 8 discards that leg and re-runs it.

See dev_docs/specs/2026-08-29-memory-path-slice-design.md section 7.5.
#>
...
$lockPath = Join-Path $bench "locks\hold-load-bias-ladder.lock"
...
"$PID hold-load-bias-ladder" | Set-Content -LiteralPath $lockPath -NoNewline
...
function Set-LadderEnvironment([string]$HoldArm) {
    $env:IZARRAVM_JIT = "1"
    ...
    $env:IZARRAVM_DIRECT_HOLD_LOAD_BIAS = $HoldArm
    foreach ($observer in @(
...
function Invoke-Leg([string]$Arm, [string]$Label) {
    ...
    # The counter is inside direct_stalls (same JSON object as jcc_sites_*). A
    # missing field must not coerce to 0: [uint64]$null -eq 0 is true in PowerShell
    # and that is how identity-on.json with 97058 probes was called vacuous.
    $probes = Get-Field $stalls "hold_load_bias_probes"
    if ($null -eq $probes) {
        throw "$Label missing direct_stalls.hold_load_bias_probes"
    }
    $hash = $null
    ...
    [pscustomobject]@{
        ...
        cpu_after                = $cpuAfter
        # Flagged at 8 (the design's prompt-to-look bar). Re-run only above 25:
        # this host's idle sits 14-23%, so a bar of 8 can never go green here.
        ...
        hold_load_bias_probes    = [uint64]$probes
        frame_sha256             = $hash
    }
}
...
try {
    ...
    foreach ($field in @("insns", "core_clocks", "raw_bus", "scaled_bus", "master_ticks", "jit_direct_insns", "jit_direct_entries")) {
        if ($off.$field -ne $on.$field) {
            $identityOk = $false
            $identityNotes.Add("$field OFF=$($off.$field) ON=$($on.$field)")
        }
    }
    if ([uint64]$off.hold_load_bias_probes -ne 0) {
        $identityOk = $false
        $identityNotes.Add("OFF hold_load_bias_probes=$($off.hold_load_bias_probes) (must be 0)")
    }
    if ([uint64]$on.hold_load_bias_probes -eq 0) {
        $identityOk = $false
        $identityNotes.Add("ON hold_load_bias_probes=0 (vacuous; knob did not engage)")
    }

    Write-Host ("IDENTITY OFF wall={0:N3}s ON wall={1:N3}s probes OFF={2} ON={3} hash={4}" -f `
            $off.wall_s, $on.wall_s, $off.hold_load_bias_probes, $on.hold_load_bias_probes, $off.frame_sha256)
    if (-not $identityOk) {
    ...
    if ($IdentityOnly) {
        $summary = [pscustomobject]@{ identity = "pass"; discards = $totalDiscards; off = $off; on = $on }
        $summary | ConvertTo-Json -Depth 6 | Set-Content (Join-Path $OutDir "identity.json")
        return
    }

    Write-Host "WALL ladder: $Pairs ABBA pairs on wolf3d-586"
    ...
    for ($p = 1; $p -le $Pairs; $p++) {
        ...
        foreach ($step in @(@("0", "off"), @("1", "on"), @("1", "on"), @("0", "off"))) {
            ...
            $legs.Add($leg)
            $pairLegs.Add($leg)
            Write-Host ("  wall {0,9:N3}s  probes {1}  cpu {2}/{3}" -f `
                    $leg.wall_s, $leg.hold_load_bias_probes, $leg.cpu_before, $leg.cpu_after)
        }
    ...
    $verdict = if ($minRatio -ge 1.060 -and $signWins -eq $signN) {
        "SHIP_DEFAULT_ON (owner decision)"
    }
    elseif ($minRatio -ge 1.020 -and $signWins -ge 7) {
        "REAL_AND_SMALL (keep default OFF)"
    }
    else {
        "DECLINE on wolf3d-586 (keep default OFF)"
    }
