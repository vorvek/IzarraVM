...
/* modified by the Toka-DOS project, 2026: explicit (ATTR) cast -- Turbo C
   accepted the implicit unsigned-int-to-byte truncation silently, Open
   Watcom's -we (warnings-as-errors) does not; behavior is unchanged. */
...
/* modified by the Toka-DOS project, 2026: stpcpy() is a GNU libc extension
   that Turbo C's runtime happened to expose but Open Watcom's DOS target
   does not declare. Portable one-line replacement, same contract (copies
   src including the NUL, returns a pointer to the copied NUL terminator). */
LOCAL char *PROC toka_stpcpy (char *dst, const char *src) {
	while ((*dst = *src) != '\0') dst++, src++;
	return dst;
}
#define stpcpy toka_stpcpy
